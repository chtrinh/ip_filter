...
Add the following method to any controller to validate the request#ip. Ipfilter::Configuration.ip_exception is raised when ip NOT within the specified region/country. You can specific actions as well similar to a before_filter. 

  validate_ip

You can also pass a block (with filter options) to be executed when validation of ip fails like so (defaults to Ipfilter::Configuration.ip_exception if no block present): 
...
Make sure all keys that are going to be in use have values, all keys ommitted will have value of nil.
